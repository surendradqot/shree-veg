...
class OrderRepo {
  ...
  Future<ApiResponse> placeOrder(PlaceOrderBody orderBody) async {
    log('Requested order body is:::::::::::::::::::::::::::::::::     ${orderBody.toJson()}');
  ...
  List<String> getDates(BuildContext context) {
    ...
    dates.add(
        DateConverter.slotDate(DateTime.now().add(const Duration(days: 1))));
    dates.add(
        DateConverter.slotDate(DateTime.now().add(const Duration(days: 2))));
  ...
  Future<ApiResponse> updatePaymentMethod(String orderID) async {
    try {
      ...
      final response = await dioClient!
          .post(AppConstants.updatePaymentMethodUri, data: data);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
