...
class WalletRepo {
  ...
  Future<ApiResponse> getWalletTransactionList(String offset) async {
    try {
      final response = await dioClient!
          .get('${AppConstants.walletTransactionUrl}?offset=$offset&limit=10');
  ...
  Future<ApiResponse> getLoyaltyTransactionList(
  ...
  Future<ApiResponse> walletTopup({String? amount}) async {
    try {
      final response = await dioClient!
          .post(AppConstants.walletTopup, data: {'amount': amount});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
