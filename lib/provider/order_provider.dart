...
import 'package:shreeveg/data/model/response/distance_model.dart';
...
import 'package:shreeveg/data/model/response/delivery_man_model.dart';
...
enum RadioGroup { walletPayment, onlinePayment, cashOnDelivery }
...
class OrderProvider extends ChangeNotifier {
  ...
  List<OrderModel>? _cancelOrderListMe;
  List<OrderModel>? _cancelOrderListDeliveryBoy;
  ...
  RadioGroup? _paymentOptionRadioGroupValue;

  List<TimeSlotModel>? get timeSlots => _timeSlots;
  List<TimeSlotModel>? get allTimeSlots => _allTimeSlots;
  List<OrderModel>? get runningOrderList => _runningOrderList;
  List<OrderModel>? get historyOrderList => _historyOrderList;
  List<OrderModel>? get cancelOrderListMe => _cancelOrderListMe;
  List<OrderModel>? get cancelOrderListDeliveryBoy =>
  ...
  bool get isCancelByMe => _isCancelByMe;
  ...
  RadioGroup? get paymentOptionRadioGroupValue => _paymentOptionRadioGroupValue;
  ...
  void validateSlot(List<TimeSlotModel>? slots, int dateIndex) {
    ...
    if (dateIndex == 0) {
      ...
      for (var slot in slots!) {
        DateTime time = DateConverter.stringTimeToDateTime(slot.endTime!)
  ...
  void setPaymentOptionRadio(RadioGroup? option) {
    _paymentOptionRadioGroupValue = option;
    if (option == RadioGroup.walletPayment) {
      setPaymentMethod('wallet_payment');
    } else if (option == RadioGroup.onlinePayment) {
  ...
  void changeCancelledOrderStatus(bool status) {
    _isCancelByMe = status;
    notifyListeners();
  }
  ...
  Future<void> placeOrder(
    ...
    print('place order api response is: ${apiResponse.response}');
  ...
  void clearPrevData() {
    ...
    _distance = -1;
    _paymentOptionRadioGroupValue = null;
  }
}
