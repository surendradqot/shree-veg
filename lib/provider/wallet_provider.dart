...
import 'package:shreeveg/data/model/response/wallet_model.dart';
...
class WalletProvider with ChangeNotifier {
  ...
  int? _selectedMoneyTab;
  int? get selectedMoneyTab => _selectedMoneyTab;

  setSelectedMoneyTab(int money) {
    _selectedMoneyTab = money;
    notifyListeners();
  }
  ...
  Future<void> getLoyaltyTransactionList(
      String offset, bool reload, bool fromWallet,
      {bool isEarning = false}) async {
    if (offset == '1' || reload) {
      _offsetList = [];
      _offset = 1;
    ...
    if (!_offsetList.contains(offset)) {
      ...
      if (fromWallet) {
        ...
        apiResponse = await walletRepo!.getLoyaltyTransactionList(
            offset, isEarning ? 'earning' : 'converted');
      }
      ...
      if (apiResponse.response != null &&
        ...
        _transactionList!
            .addAll(WalletModel.fromJson(apiResponse.response!.data).data!);
  ...
  Future<bool> addToWallet(String amount) async {
    bool isSuccess = false;
    _isLoading = true;
    ApiResponse apiResponse = await walletRepo!.walletTopup(amount: amount);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      isSuccess = true;
      Provider.of<ProfileProvider>(Get.context!, listen: false).getUserInfo();
