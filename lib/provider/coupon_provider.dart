...
import 'package:shreeveg/data/model/response/coupon_model.dart';
    ...
    if (apiResponse.response != null && apiResponse.response!.data != null) {
      ...
      if (_coupon!.minPurchase != null && _coupon!.minPurchase! <= order) {
        if (_coupon!.discountType == 'percent' &&
            _coupon!.couponType != 'free_delivery') {
          ...
          showCustomSnackBar(
              '${'you_got_discount'.tr} ${'${_coupon!.discount}%'}',
              isError: false);
        } else if (_coupon!.discountType == 'amount' &&
            order < _coupon!.discount!) {
          ...
          showCustomSnackBar(
              '${'you_got_discount'.tr} ${PriceConverter.convertPrice(Get.context!, _coupon!.discount)}',
              isError: false);
        }
      } else {
        showCustomSnackBar('${'you_need_order_more_than'.tr} '
