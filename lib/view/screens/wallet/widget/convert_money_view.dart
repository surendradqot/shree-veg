...
class _ConvertMoneyViewState extends State<ConvertMoneyView> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<String?> noteList = [
      ...
      '${Provider.of<SplashProvider>(context, listen: false).configModel!.loyaltyPointExchangeRate} ${getTranslated('point', context)} ${getTranslated('remain', context)} ${PriceConverter.convertPrice(context, 1)}',
          ...
          children: [
            ...
            Consumer<WalletProvider>(builder: (context, walletProvider, _) {
              return walletProvider.isLoading
                  ...
                  : CustomButton(
                      borderRadius: 30,
                      buttonText: getTranslated('convert_point', context),
                      onPressed: () {
                        if (_pointController.text.isEmpty) {
                          showCustomSnackBar(getTranslated(
                              'please_enter_your_point', context)!);
                        } else {
                          int point = int.parse(_pointController.text.trim());

                          if (point < configModel!.loyaltyPointMinimumPoint!) {
                            showCustomSnackBar(
                                '${getTranslated('please_exchange_more_then', context)!} ${configModel.loyaltyPointMinimumPoint} ${getTranslated('points', context)!}');
                          } else {
                            walletProvider.pointToWallet(point, false).then(
