...
class OfflinePaymentDialog extends StatefulWidget {
  ...
  const OfflinePaymentDialog(
      {Key? key, required this.placeOrderBody, this.callBack})
      : super(key: key);
                          ...
                          onPressed: () {
                            if (paymentByController.text.isEmpty) {
                              showCustomSnackBar(getTranslated(
                                  'please_insert_user_name', context)!);
                            } else if (transactionIdController.text.isEmpty) {
                              showCustomSnackBar(getTranslated(
                                  'please_input_transaction_id', context)!);
                            } else {
                              widget.callBack!(
                                  widget.placeOrderBody.setOfflinePayment(
