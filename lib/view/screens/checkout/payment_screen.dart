...
import 'package:shreeveg/view/screens/checkout/widget/cancel_dialog.dart';
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    super.initState();
    selectedUrl =
        '${AppConstants.baseUrl}/payment-mobile?token=${widget.url}&&payment_method=${Provider.of<OrderProvider>(context, listen: false).paymentMethod}';
    _initData();
  }
...
class MyInAppBrowser extends InAppBrowser {
  ...
  void onExit() {
    if (_canRedirect) {
      Navigator.pushReplacementNamed(context,
          '${RouteHelper.orderSuccessful}/${orderModel!.id}/payment-fail');
    }
  ...
  void _pageRedirect(String url) {
    if (_canRedirect) {
      bool isUrl =
          url.contains('${AppConstants.baseUrl}${RouteHelper.orderSuccessful}');
      ...
      if (isSuccess) {
        String token = url.replaceAll(
            '${AppConstants.baseUrl}${RouteHelper.orderSuccessful}/success?token=',
            '');
        ...
        if (token != '') {
          ...
          if (placeOrderData != null) {
            Provider.of<OrderProvider>(context, listen: false)
                .placeOrder(placeOrderData, _callback);
          }
        } else {
          Navigator.pushReplacementNamed(context,
              '${RouteHelper.orderSuccessful}/${orderModel!.id}/payment-fail');
